...
const StudentIntake = () => {
  ...
  const matches = useMemo(() => tutors.filter((tutor) => {
    ...
    const maxRate = form.budgetPreference === 'free' ? 0 : Number(form.budgetPreference.replace('under-', '')) || 25;
  ...
  return (
    <AppShell><main className="page intake-page">
      ...
      {waitlist?.status === 'matched' ? <div className="waitlist-banner waitlist-banner-matched"><CheckCircle2 size={22} /><div><strong>We found a possible match</strong><p>{waitlist.matched_tutor_name} has been invited to review your tutoring request.</p></div><Link className="btn btn-primary btn-sm" to={`/tutors/${waitlist.matched_tutor_id}`}>View tutor <ArrowRight size={16} /></Link></div> : null}
