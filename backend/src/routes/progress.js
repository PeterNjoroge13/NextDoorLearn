...
const getStudentAccess = async (req, requestedStudentId) => {
...
const hydrateGoals = async (studentId) => {
  ...
  const milestonesForGoal = await db.prepare(`
  ...
  return Promise.all(goals.map(async (goal) => ({
    ...goal,
    milestones: (await milestonesForGoal.all(goal.id)).map((milestone) => ({
      ...milestone,
...
const refreshGoalProgress = async (goalId) => {
...
router.get('/goals', authenticateToken, async (req, res) => {
  try {
    const studentId = await getStudentAccess(req, req.query.studentId);
    if (!studentId) {
    ...
    const student = await db.prepare('SELECT id, name, avatar_url FROM users WHERE id = ? AND role = ?').get(studentId, 'student');
    if (!student) return res.status(404).json({ error: 'Student not found' });
    res.json({ student, goals: await hydrateGoals(studentId) });
  } catch (error) {
...
router.post('/goals', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(201).json((await hydrateGoals(req.user.userId)).find((goal) => Number(goal.id) === Number(result.lastInsertRowid)));
  } catch (error) {
...
router.patch('/goals/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const goal = await db.prepare('SELECT * FROM learning_goals WHERE id = ? AND student_id = ?').get(req.params.id, req.user.userId);
    ...
    res.json((await hydrateGoals(req.user.userId)).find((item) => Number(item.id) === Number(goal.id)));
  } catch (error) {
...
router.delete('/goals/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!isPositiveInteger(req.params.id)) return res.status(400).json({ error: 'Valid goal ID is required' });
    const result = await db.prepare('DELETE FROM learning_goals WHERE id = ? AND student_id = ?').run(req.params.id, req.user.userId);
...
router.post('/goals/:id/milestones', authenticateToken, async (req, res) => {
  try {
    ...
    const goal = await db.prepare('SELECT id FROM learning_goals WHERE id = ? AND student_id = ?').get(req.params.id, req.user.userId);
    if (!goal) return res.status(404).json({ error: 'Learning goal not found' });
    const result = await db.prepare('INSERT INTO goal_milestones (goal_id, title) VALUES (?, ?)').run(goal.id, title);
    await refreshGoalProgress(goal.id);
    res.status(201).json(await db.prepare('SELECT * FROM goal_milestones WHERE id = ?').get(result.lastInsertRowid));
  } catch (error) {
...
router.patch('/milestones/:id', authenticateToken, async (req, res) => {
  try {
    ...
    await refreshGoalProgress(milestone.goal_id);
    res.json((await hydrateGoals(req.user.userId)).find((goal) => Number(goal.id) === Number(milestone.goal_id)));
  } catch (error) {
...
router.delete('/milestones/:id', authenticateToken, async (req, res) => {
  try {
    ...
    `).get(req.params.id, req.user.userId);
    if (!milestone) return res.status(404).json({ error: 'Milestone not found' });
    await db.prepare('DELETE FROM goal_milestones WHERE id = ?').run(milestone.id);
    await refreshGoalProgress(milestone.goal_id);
