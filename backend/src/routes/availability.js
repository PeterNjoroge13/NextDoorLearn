...
router.put('/me', authenticateToken, async (req, res) => {
  try {
    ...
    const replaceAvailability = async (rows) => db.withTransaction(async (transactionDb) => {
      await transactionDb.prepare('DELETE FROM tutor_availability_slots WHERE tutor_id = ?').run(userId);
      if (rows.length > 0) {
        ...
        for (const row of rows) {
          await insert.run(userId, row.dayOfWeek, row.startTime, row.endTime, row.timezone);
        }
      }
    });
...
router.get('/tutor/:tutorId', authenticateToken, async (req, res) => {
  try {
