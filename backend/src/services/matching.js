...
const getTutorRecommendations = async ({ studentId, student = {}, limit = 12 }) => {
  ...
  for (const tutor of tutors) {
    ...
    recommendations.push({
      ...
      ...scoreTutor(tutor, student, slots)
    });
  }
  recommendations.sort((a, b) => b.matchScore - a.matchScore || b.totalReviews - a.totalReviews);
  return recommendations.slice(0, Math.min(Math.max(Number(limit) || 12, 1), 25));
};

module.exports = { getTutorRecommendations, normalized, parseList, scoreTutor };
