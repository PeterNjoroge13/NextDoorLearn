  ...
  return <View style={styles.page}>
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.actions}>
        <Pressable style={styles.primary} onPress={() => router.push('/(auth)/signup')}><Text style={styles.primaryText}>Find a tutor</Text></Pressable>
        <Pressable style={styles.secondary} onPress={() => router.push('/(auth)/login')}><LogIn size={18} color={colors.ink} /><Text style={styles.secondaryText}>Sign in</Text></Pressable>
        <View style={styles.inline}><Pressable onPress={() => router.push('/(auth)/apply-tutor')}><Text style={styles.link}>Volunteer as a tutor</Text></Pressable><Text style={styles.dot}>•</Text><Pressable style={styles.sponsorLink} onPress={() => router.push('/(auth)/donate')}><HeartHandshake size={15} color={colors.white} /><Text style={styles.link}>Sponsor access</Text></Pressable></View>
      </View>
    </SafeAreaView>
  </View>;
}
...
const styles = StyleSheet.create({
  page: { flex: 1, backgroundColor: colors.ink }, hero: { ...StyleSheet.absoluteFill }, scrim: { ...StyleSheet.absoluteFill, backgroundColor: 'rgba(16, 28, 24, 0.55)' }, safe: { flex: 1, padding: spacing.xl, justifyContent: 'space-between' },
  ...
  copy: { gap: spacing.md, marginTop: 'auto', marginBottom: spacing.xl }, eyebrow: { color: '#BFE8E1', fontFamily: typography.bold, fontSize: 12, letterSpacing: 0 }, title: { color: colors.white, fontFamily: typography.bold, fontSize: 39, lineHeight: 44, letterSpacing: 0 }, body: { color: '#F4F1EA', fontFamily: typography.regular, fontSize: 17, lineHeight: 25 },
