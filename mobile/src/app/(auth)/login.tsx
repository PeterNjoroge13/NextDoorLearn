...
export default function LoginScreen() {
  ...
  return <Screen><Pressable onPress={() => router.back()} style={styles.back}><ArrowLeft size={22} color={colors.ink} /></Pressable><View style={styles.mark}><BookOpen color={colors.white} size={25} /></View><View style={styles.heading}><Text style={styles.title}>Welcome back.</Text><Text style={styles.subtitle}>Your tutors, students, messages, and plans are waiting.</Text></View>{error ? <ErrorNotice message={error} /> : null}<View style={styles.form}><Field label="Email" value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" autoComplete="email" /><Field label="Password" value={password} onChangeText={setPassword} secureTextEntry autoComplete="current-password" /><Button label="Sign in" onPress={submit} loading={loading} disabled={!email || !password} /></View><Pressable onPress={() => router.push('/(auth)/forgot-password')}><Text style={styles.link}>Forgot your password?</Text></Pressable><View style={styles.switch}><Text style={styles.switchCopy}>New to NextDoorLearn?</Text><Pressable onPress={() => router.push('/(auth)/signup')}><Text style={styles.link}>Create a student account</Text></Pressable></View></Screen>;
}
