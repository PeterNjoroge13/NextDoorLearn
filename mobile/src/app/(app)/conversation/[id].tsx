...
export default function ConversationScreen() {
  ...
  const reportUser = () => {
    ...
    router.push({
      ...
      params: { id: params.userId, name: params.name || 'this user' },
    });
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await request(`/blocks/${params.userId}`, {
                method: 'POST',
                body: JSON.stringify({ reason: 'Blocked from conversation' }),
              });
              router.replace('/(app)/(tabs)/messages');
            } catch (caught) {
              setError(caught instanceof Error ? caught.message : 'This user could not be blocked.');
            }
          },
        },
      ],
  ...
  const showSafetyMenu = () => Alert.alert(
    params.name || 'Conversation safety',
    'Reports are private. Blocking immediately stops matching and communication.',
    [
      { text: 'Cancel', style: 'cancel' },
  ...
  return <SafeAreaView style={styles.safe}>
    <View style={styles.header}>
      ...
      <Avatar name={params.name || 'Conversation'} size={38} />
      <View style={styles.heading}>
        <Text style={styles.name}>{params.name || 'Conversation'}</Text>
        <Text style={styles.status}>NextDoorLearn connection</Text>
      </View>
      {params.userId ? <Pressable accessibilityRole="button" accessibilityLabel="Conversation safety options" onPress={showSafetyMenu} style={styles.headerButton}>
        <MoreVertical size={21} color={colors.ink} />
      </Pressable> : null}
    </View>
    ...
    <FlatList
      ...
      renderItem={({ item }) => {
        ...
        return <View style={[styles.bubble, mine ? styles.mine : styles.theirs]}>
          <Text style={[styles.body, mine && styles.mineBody]}>{item.content}</Text>
          <Text style={[styles.time, mine && styles.mineTime]}>{formatTime(item.timestamp)}</Text>
        </View>;
      }}
      ListEmptyComponent={<View style={styles.empty}>
        <Text style={styles.emptyTitle}>Start the conversation</Text>
        <Text style={styles.emptyCopy}>A simple introduction is a good place to begin.</Text>
      </View>}
    />
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.composer}>
        ...
        <Pressable
          ...
          onPress={send}
          style={[styles.send, (!message.trim() || sending) && styles.disabled]}
        >
...
const styles = StyleSheet.create({
  ...
  header: { minHeight: 68, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingHorizontal: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.line, backgroundColor: colors.surface },
  headerButton: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' },
  heading: { flex: 1 },
  name: { color: colors.ink, fontFamily: typography.bold, fontSize: 15 },
  ...
  body: { color: colors.ink, fontFamily: typography.regular, fontSize: 15, lineHeight: 21 },
  mineBody: { color: colors.white },
  time: { color: colors.muted, fontSize: 9 },
  mineTime: { color: '#C9E8E3', textAlign: 'right' },
  composer: { flexDirection: 'row', alignItems: 'flex-end', gap: spacing.sm, padding: spacing.md, borderTopWidth: 1, borderTopColor: colors.line, backgroundColor: colors.surface },
  input: { flex: 1, minHeight: 44, maxHeight: 110, borderWidth: 1, borderColor: colors.line, borderRadius: 22, paddingHorizontal: spacing.lg, paddingTop: 11, color: colors.ink, backgroundColor: colors.background, fontSize: 15 },
  ...
  emptyTitle: { color: colors.ink, fontFamily: typography.bold, fontSize: 18 },
