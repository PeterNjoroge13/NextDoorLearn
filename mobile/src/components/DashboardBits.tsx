...
export function SessionCard({ session }: { session: Session }) {
  ...
  return <Card style={styles.itemCard}><View style={styles.date}><Text style={styles.month}>{date.toLocaleDateString(undefined, { month: 'short' }).toUpperCase()}</Text><Text style={styles.day}>{date.getDate()}</Text></View><View style={styles.itemCopy}><Text style={styles.itemTitle}>{session.title}</Text><Text style={styles.meta}>{session.start_time?.slice(0, 5)} – {session.end_time?.slice(0, 5)} · {session.tutor_name || session.student_name || session.subject || 'Tutoring'}</Text>{session.session_timezone ? <Text style={styles.timezone}>{session.session_timezone}</Text> : null}</View><Chip label={session.confirmation_status === 'pending' ? 'Pending' : session.status} tone={session.confirmation_status === 'pending' ? 'gold' : 'brand'} /></Card>;
}
